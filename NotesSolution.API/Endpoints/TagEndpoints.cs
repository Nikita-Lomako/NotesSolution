...
namespace NotesSolution.API.Endpoints
{
    public static class TagEndpoints
    {
        public static void MapTagEndpoints(this IEndpointRouteBuilder app)
        {
            ...
            group.MapPost("/", CreateTag).WithName("CreateTag").Accepts<TagDto>("application/json").Produces<TagDto>(201).Produces(400);
            group.MapPut("/{id}", UpdateTag).WithName("UpdateTag").Accepts<TagDto>("application/json").Produces<TagDto>(200).Produces(400).Produces(404);
            group.MapDelete("/{id}", DeleteTag).WithName("DeleteTag").Produces(204).Produces(404);
            group.MapGet("/{id}", GetTagById).WithName("GetTagById").Produces<TagDto>(200).Produces(404);
            group.MapGet("/by-name/{name}", GetTagByName).WithName("GetTagByName").Produces<TagDto>(200).Produces(404);
        }
            ...
            ILogger<Program> logger)
        {
            ...
            if (tag == null)
            {
                logger.LogWarning($"Tag with name {name} not found");
                return Results.NotFound();
            }
            var tagDto = mapper.Map<TagDto>(tag);
            return Results.Ok(tagDto);
